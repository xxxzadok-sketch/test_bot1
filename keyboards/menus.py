...
def get_user_booking_filter_menu():
    keyboard = [
        [KeyboardButton("⏳ Ожидающие"), KeyboardButton("📅 По дате")],
        [KeyboardButton("⬅️ Назад")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
...
# Меню фильтрации бронирований для администратора (ИЗМЕНЕНО)
def get_booking_filter_menu():
    """
    Меню фильтрации бронирований для администратора
    Убраны кнопки: подтвержденные, отмененные, все бронирования
    Добавлена кнопка: забронировать
    """
    keyboard = [
        [KeyboardButton("⏳ Ожидающие"), KeyboardButton("📅 По дате")],
        [KeyboardButton("➕ Забронировать"), KeyboardButton("⬅️ Назад")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
...
# Клавиатура для управления бронированиями администратором (ИЗМЕНЕНО - теперь только для ожидающих)
