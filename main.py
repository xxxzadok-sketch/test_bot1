...
def setup_handlers(application):
    ...
    async def catch_all_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Ловит все callback для отладки"""
        query = update.callback_query
        print(f"🔥 ВСЕ CALLBACK: {query.data}")
        logger.info(f"🔥 ВСЕ CALLBACK: {query.data}")
        # Не отвечаем, чтобы другие обработчики могли сработать

    # Добавляем этот обработчик САМЫМ ПЕРВЫМ с высоким приоритетом
    ...
    from handlers.user_handlers import (
        ...
        get_user_booking_date_filter_handler,  # <-- ИСПРАВЛЕНО: добавлена запятая
    )
    ...
    from handlers.admin_bookings import (
        ...
        get_admin_booking_handler  # <-- ДОБАВЬТЕ ЭТУ СТРОКУ
    )
    ...
    application.add_handler(get_admin_booking_handler())  # <-- НОВЫЙ обработчик для создания брони админом
    ...
    application.add_handler(get_user_booking_date_filter_handler())  # <-- НОВЫЙ ОБРАБОТЧИК
